...
class AjaxCart extends HTMLElement {
    constructor() {
      ...
      document.querySelectorAll('[data-gwp-close]').forEach(button => button.addEventListener('click', this.closePopup.bind(this)));
      document.querySelectorAll('[add-gwp-product]').forEach(button => button.addEventListener('click', this.addMultiGWPProduct.bind(this)));
    ...
    updateEvents(){
      ...
      this.querySelectorAll('[data-gwp-multi-tier]').forEach(button => button.addEventListener('click', this.openPopup.bind(this)));
    ...
    openPopup(event){
        ...
        document.getElementById(getDivID).querySelector('.modal').classList.add('open');
    ...
    _updateCart(response, action){
      ...
      let removeTier1Element = document.querySelector('[data-gwp-remove]');
        if (typeof(removeTier1Element) != 'undefined' && removeTier1Element != null){
      ...
      if(window.globalVariables.cart.item_count <= 0){
        ...
        if(document.querySelector('[data-upsell-product]')){
          document.querySelector('[data-upsell-product]').classList.remove('d-none');
        }
      }
      ...
      if(window.globalVariables.cart.item_count > 0){
        if(headerHTML.querySelector('#cart-icon-desktop .cart-count')){
          if(drawerSelectors.cartDrawerCount) drawerSelectors.cartDrawerCount.innerHTML = headerHTML.querySelector('#cart-icon-desktop .cart-count').innerHTML;
        }
      }
    ...
    addMultiGWPProduct(event) {
        ...
        let upsellProductVariantId = currentTarget.getAttribute("add-gwp-product") || null;
        ...
        var allBtns = currentTarget.closest('.modal').querySelectorAll('[add-gwp-product]');
